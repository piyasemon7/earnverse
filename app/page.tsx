...
<div style={{padding: '50px 40px', background: '#1a1a2e'}}>
  <div style={{maxWidth: '1500px', margin: '0 auto'}}>
    ...
    <div style={{display: 'flex', flexWrap: 'wrap', gap: '12px', justifyContent: 'center'}}>
      ...
      {/* ── FAQ ── */}
      <FaqSection />
      {/* About & Other Sites Section */}
<div style={{padding: '60px 40px', background: '#f8f8f8'}}>
  <div style={{maxWidth: '1500px', margin: '0 auto'}}>
    <h2 style={{textAlign: 'center', color: '#1a1a2e', fontSize: '32px', fontWeight: 'bold', marginBottom: '8px'}}>
      👨‍💻 About The Author
    </h2>
    <p style={{textAlign: 'center', color: '#777', marginBottom: '40px', fontSize: '16px'}}>
      The person behind EarnVerse
    </p>
    ...
    <div style={{display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '40px', alignItems: 'start'}}>
      ...
      <div style={{
        ...
        textAlign: 'center', border: '2px solid #FFD70033',
        ...
        <div style={{position: 'relative', display: 'inline-block', marginBottom: '20px'}}>
          <img
            ...
            style={{
              ...
              objectFit: 'cover', border: '4px solid #FFD700',
              display: 'block'
            }}
          />
          <span style={{
            position: 'absolute', bottom: '5px', right: '5px',
            background: '#00b894', width: '20px', height: '20px',
            borderRadius: '50%', border: '3px solid #1a1a2e',
            display: 'block'
          }} />
        </div>
        <h3 style={{color: '#FFD700', fontSize: '22px', fontWeight: 'bold', margin: '0 0 5px 0'}}>
          আপনার নাম
        </h3>
        <p style={{color: '#aaa', fontSize: '14px', margin: '0 0 5px 0'}}>
          Founder & Author
        </p>
        <p style={{color: '#FFD700', fontSize: '13px', margin: '0 0 20px 0'}}>
          📍 Bangladesh
        </p>
        <p style={{color: '#bbb', fontSize: '13px', lineHeight: '1.7', marginBottom: '20px'}}>
          Digital entrepreneur, blogger and online earning expert. Helping people earn money online since 2020.
        </p>
        <div style={{display: 'flex', justifyContent: 'center', gap: '10px'}}>
          {[
            { label: '50+', sub: 'Articles' },
            { label: '10K+', sub: 'Readers' },
            { label: '4+', sub: 'Years' },
          ].map((stat, i) => (
            <div key={i} style={{
              ...
              <div style={{color: '#FFD700', fontWeight: 'bold', fontSize: '16px'}}>{stat.label}</div>
              <div style={{color: '#aaa', fontSize: '11px'}}>{stat.sub}</div>
            </div>
          ))}
        </div>
      </div>

      {/* Right - Other Websites */}
      <div>
        <h3 style={{color: '#1a1a2e', fontSize: '20px', fontWeight: 'bold', marginBottom: '20px'}}>
          🌐 My Other Websites
        </h3>
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px'}}>
          {[
            { name: "Website Name 1", desc: "Short description of this website goes here.", url: "#", color: "#6c63ff", icon: "🌍" },
            { name: "Website Name 2", desc: "Short description of this website goes here.", url: "#", color: "#00b894", icon: "💻" },
            { name: "Website Name 3", desc: "Short description of this website goes here.", url: "#", color: "#e17055", icon: "📱" },
            { name: "Website Name 4", desc: "Short description of this website goes here.", url: "#", color: "#fdcb6e", icon: "🎯" },
          ].map((site, i) => (
            <a key={i} href={site.url} target="_blank" rel="noreferrer" style={{
              background: 'white', borderRadius: '14px', padding: '20px',
              border: `2px solid ${site.color}33`,
              textDecoration: 'none', display: 'block',
              boxShadow: '0 3px 15px rgba(0,0,0,0.06)',
              transition: 'all 0.3s'
            }}
            onMouseEnter={e => {
              (e.currentTarget as HTMLAnchorElement).style.borderColor = site.color;
              (e.currentTarget as HTMLAnchorElement).style.transform = 'translateY(-5px)';
              (e.currentTarget as HTMLAnchorElement).style.boxShadow = `0 10px 25px ${site.color}33`;
            }}
            onMouseLeave={e => {
              (e.currentTarget as HTMLAnchorElement).style.borderColor = `${site.color}33`;
              (e.currentTarget as HTMLAnchorElement).style.transform = 'translateY(0)';
              (e.currentTarget as HTMLAnchorElement).style.boxShadow = '0 3px 15px rgba(0,0,0,0.06)';
            }}>
              <div style={{display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px'}}>
                <span style={{
                  fontSize: '28px', width: '50px', height: '50px',
                  background: `${site.color}15`, borderRadius: '12px',
                  display: 'flex', alignItems: 'center', justifyContent: 'center'
                }}>
                  {site.icon}
                </span>
                <div>
                  <h4 style={{color: '#1a1a2e', fontSize: '15px', fontWeight: 'bold', margin: 0}}>
                    {site.name}
                  </h4>
                  <span style={{
                    color: site.color, fontSize: '11px', fontWeight: '600'
                  }}>
                    Visit Website →
                  </span>
                </div>
